...
const getKey = ({ directory, file }) => {
  const prefix = ['thumbnail', 'large', 'medium', 'small'].find(p => file.name.startsWith(p)) || '';
  const new_name = prefix ? prefix : '';
  const file_name_hash = crypto.createHash("md5").update(file.name).digest("hex");
  return `${directory}/${file_name_hash}/${new_name}_${file.hash}${file.ext}`.replace(/^\//g, "");
};
